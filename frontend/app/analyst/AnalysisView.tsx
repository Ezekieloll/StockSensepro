...
import { ChartIcon, SearchIcon, CheckIcon, CalendarIcon, DatabaseIcon } from '@/components/Icons';
                ...
                <g transform={`translate(${padding.left}, ${padding.top})`}>
                    ...
                    {selectedProducts.map((sku, idx) => {
                        ...
                        if (chartType === 'bar') {
                            ...
                            // Dynamic bar width
                            const totalBars = chartData.length * selectedProducts.length;
                ...
                <Card glass className="h-full">
                    ...
                    <CardContent className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 gap-2">
                                <div>
                                    ...
                                    <div className="relative">
                                        <CalendarIcon className="absolute left-3 top-2 text-muted" size={14} />
                                        <input
                                            type="date"
                                            value={startDate}
                                            onChange={(e) => setStartDate(e.target.value)}
                                            className="w-full bg-slate-900 border border-white/20 rounded-lg py-1.5 pl-9 pr-2 text-sm text-white focus:outline-none focus:border-primary"
                                        />
                                    </div>
                                </div>
                                <div>
                        ...
                        <div className="space-y-2 flex-1 flex flex-col min-h-0">
                            <label className="text-xs font-bold text-muted uppercase tracking-wider">Product Selection</label>
                            <div className="relative mb-2">
                                <SearchIcon className="absolute left-3 top-2 text-muted" size={14} />
                                <input
                                    type="text"
                                    placeholder="Filter products..."
                                    value={searchQuery}
                                    onChange={(e) => setSearchQuery(e.target.value)}
                                    className="w-full bg-slate-900 border border-white/20 rounded-lg py-1.5 pl-9 pr-2 text-xs text-white focus:outline-none focus:border-primary"
                                />
                            </div>
                            <div className="border border-white/10 rounded-lg bg-slate-900/50 flex-1 overflow-y-auto max-h-[300px] p-2 space-y-1">
                                {products
                                    ...
                                    .map(p => (
                                        <div
                                            ...
                                            className={`flex items-center gap-2 p-2 rounded cursor-pointer transition-colors ${selectedProducts.includes(p.sku) ? 'bg-primary/20 border border-primary/30' : 'hover:bg-white/5 border border-transparent'
                                                }`}
                                        >
                                            <div className={`w-4 h-4 rounded border flex items-center justify-center ${selectedProducts.includes(p.sku) ? 'bg-primary border-primary' : 'border-white/20'
                                                }`}>
                                                {selectedProducts.includes(p.sku) && <CheckIcon size={10} className="text-white" />}
                                            </div>
                                            <div>
                            ...
                            <p className="text-[10px] text-muted text-right">{selectedProducts.length} / 5 selected</p>
                        </div>
                    </CardContent>
                </Card>
            </div>
                ...
                <Card glass className="h-full flex flex-col">
                    <CardHeader className="border-b border-white/10 bg-white/5">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                            <div>
                                ...
                                <CardDescription>Comparing {selectedProducts.length} products across {selectedStore}</CardDescription>
                            </div>
                            ...
                            <div className="flex items-center gap-2 bg-slate-900/50 p-1 rounded-lg border border-white/10">
                                <button
                                    ...
                                    className={`px-3 py-1.5 text-xs font-medium rounded-md transition-all ${chartType === 'line' ? 'bg-primary text-white shadow-sm' : 'text-muted hover:text-white'
                                        }`}
                                >
                                    Line
                                </button>
                                <button
                                    ...
                                    className={`px-3 py-1.5 text-xs font-medium rounded-md transition-all ${chartType === 'bar' ? 'bg-primary text-white shadow-sm' : 'text-muted hover:text-white'
                                        }`}
                                >
                                    Bar
                                </button>
                                <button
                                    ...
                                    className={`px-3 py-1.5 text-xs font-medium rounded-md transition-all ${chartType === 'area' ? 'bg-primary text-white shadow-sm' : 'text-muted hover:text-white'
                                        }`}
                                >
                                    Area
                                </button>
                    ...
                    <CardContent className="flex-1 p-6 relative min-h-[400px]">
                        ...
                        <div className="flex flex-wrap gap-4 mb-6 justify-center">
                            {selectedProducts.map((sku, i) => {
                                const product = products.find(p => p.sku === sku);
                                return (
                                    <div key={sku} className="flex items-center gap-2 text-xs bg-slate-900/50 px-2 py-1 rounded-full border border-white/10">
                                        <span className="w-3 h-3 rounded-full" style={{ backgroundColor: colors[i % colors.length] }}></span>
                                        <span className="font-medium">{product?.name || sku}</span>
                                    </div>
                                );
                            })}
                        </div>
