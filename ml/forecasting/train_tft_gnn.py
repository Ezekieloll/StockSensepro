"""
TFT + GNN Training Script

This script trains the combined TFT+GNN model that:
1. Uses temporal patterns from TFT (LSTM + attention)
2. Enriches predictions with product relationships from GNN
3. Leverages the improved co-purchase graph

Based on claude_code.py with GNN integration.
"""
...
from forecasting.tft_gnn_model import TFTWithGNN, TFTWithGNNLight
...
def train_epoch(model, loader, optimizer, criterion, device, adj, clip_grad=1.0, use_gnn=True):
    """Train one epoch with GNN support."""
    model.train()
    losses = []

    for x, y, sku_indices in loader:
        x = x.to(device)
        y = y.to(device)
        ...
        # Forward with GNN
        preds = model(x, sku_indices=sku_indices, adj=adj, use_gnn=use_gnn)
...
def eval_epoch(model, loader, device, adj, use_gnn=True):
...
def main():
    ...
    # HYPERPARAMETERS
    # ==========================================
    WINDOW_SIZE = 30          # History window
    NUM_FEATURES = 9          # Input features
    HIDDEN_SIZE = 64          # Reduced for faster training with GNN
    LSTM_LAYERS = 2
    NUM_GNN_LAYERS = 2
    NUM_PRODUCTS = 240
    ...
    CLIP_GRAD = 1.0
    PATIENCE = 15
    USE_GNN = True            # Toggle GNN on/off
    USE_LIGHT_MODEL = True    # Use lighter model for faster training
    ...
    # MODEL
    # ==========================================
    print(f"\n🤖 Building TFT+GNN model...")
    
    if USE_LIGHT_MODEL:
        model = TFTWithGNNLight(
    ...
    # ==========================================
    print("\n" + "="*80)
    print(f"🚀 STARTING TRAINING (GNN={'ON' if USE_GNN else 'OFF'})")
    print("="*80)
    ...
    # Load best model
    checkpoint = torch.load('models/best_tft_gnn_model.pt')
    model.load_state_dict(checkpoint['model_state_dict'])
    
    # Final validation
    final_metrics = eval_epoch(model, val_loader, device, adj, USE_GNN)
    
    print("\n📊 FINAL VALIDATION METRICS (TFT+GNN):")
    ...
    if USE_GNN and adj is not None:
        print("\n🔬 Comparing WITH vs WITHOUT GNN...")
        no_gnn_metrics = eval_epoch(model, val_loader, device, adj, use_gnn=False)
        
        print(f"\n   {'Metric':<20} {'With GNN':<15} {'Without GNN':<15} {'Difference':<15}")
        print("   " + "-"*60)
        for metric in ['mae', 'mape', 'wape']:
            ...
            print(f"   {metric.upper():<20} {with_gnn:<15.2f} {without_gnn:<15.2f} {diff:+.2f} {symbol}")
    ...
    # Save final results
    with open('models/tft_gnn_final_results.json', 'w') as f:
        json.dump({
    ...
    print("✅ Results saved to models/tft_gnn_final_results.json")
