import torch
import torch.nn as nn
from torch.utils.data import DataLoader
import numpy as np

from forecasting.dataset import DemandDataset
from forecasting.lstm_gnn_model import LSTMGNNModel
from gnn.graph_utils import build_graph_tensors


# --------------------
# Metrics
# --------------------
def rmse(y_true, y_pred):
    y_true = np.array(y_true)
    y_pred = np.array(y_pred)
    return np.sqrt(np.mean((y_true - y_pred) ** 2))


def mape(y_true, y_pred):
    y_true = np.array(y_true)
    y_pred = np.array(y_pred)

    mask = y_true != 0
    return np.mean(np.abs((y_true[mask] - y_pred[mask]) / y_true[mask])) * 100

def wape(y_true, y_pred):
    y_true = np.array(y_true)
    y_pred = np.array(y_pred)
    denom = y_true.sum()
    if denom == 0:
        return np.nan
    return np.sum(np.abs(y_true - y_pred)) / denom * 100

# --------------------
# Training
# --------------------
def train_epoch(model, loader, optimizer, criterion, adj, node_to_idx, device):
    model.train()
    total_loss = 0.0

    for x, y, product_ids in loader:
        x = x.to(device)
        y = y.to(device)

        sku_idx = torch.tensor(
            [node_to_idx[p] for p in product_ids],
            device=device,
            dtype=torch.long,
        )

        optimizer.zero_grad()
        preds = model(x, sku_idx, adj)
        loss = criterion(preds, y)
        loss.backward()
        optimizer.step()

        total_loss += loss.item() * len(x)

    return total_loss / len(loader.dataset)


def eval_epoch(model, loader, adj, node_to_idx, device):
    model.eval()
    preds_all = []
    y_all = []

    with torch.no_grad():
        for x, y, product_ids in loader:
            x = x.to(device)

            sku_idx = torch.tensor(
                [node_to_idx[p] for p in product_ids],
                device=device,
                dtype=torch.long,
            )

            preds = model(x, sku_idx, adj)
            preds_all.extend(preds.cpu().numpy())
            y_all.extend(y.numpy())

    return rmse(y_all, preds_all), mape(y_all, preds_all), wape(y_all, preds_all)


# --------------------
# Main
# --------------------
def main():
    device = torch.device("cpu")

    # Build graph tensors
    nodes, node_to_idx, adj = build_graph_tensors(
        "data/raw/categories_products.csv"
    )
    adj = adj.to(device)

    # Datasets
    train_ds = DemandDataset("data/processed/train.csv", window_size=17)
    val_ds = DemandDataset("data/processed/val.csv", window_size=17)

    train_loader = DataLoader(train_ds, batch_size=64, shuffle=True)
    val_loader = DataLoader(val_ds, batch_size=64, shuffle=False)

    # Model
    model = LSTMGNNModel(input_size=4, hidden_size=32).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=0.001)
    criterion = nn.MSELoss()

    epochs = 20

    for epoch in range(1, epochs + 1):
        train_loss = train_epoch(
            model, train_loader, optimizer, criterion, adj, node_to_idx, device
        )
        val_rmse, val_mape, val_wape = eval_epoch(
            model, val_loader, adj, node_to_idx, device
        )

        print(
            f"Epoch {epoch} | "
            f"Train MSE: {train_loss:.4f} | "
            f"Val RMSE: {val_rmse:.4f} | "
            f"Val MAPE: {val_mape:.2f}% | "
            f"Val WAPE: {val_wape:.2f}%"
        )


if __name__ == "__main__":
    main()
