"""
Transformer Training Script V2 (Upgraded)
...
from forecasting.dataset_v3 import DemandDatasetV3, collate_with_sku
from forecasting.transformer_model import TransformerBaseline
...
def main():
    ...
    # Data
    print("\n📂 Loading datasets...")
    train_ds = DemandDatasetV3("data/processed2/train.csv", window_size=WINDOW_SIZE)
    val_ds   = DemandDatasetV3("data/processed2/val.csv",   window_size=WINDOW_SIZE)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if val_metrics['wape'] < best_wape:
            ...
            torch.save({
                ...
                'config': {
                    'd_model': 64, 'nhead': 4,
                    'num_layers': 2, 'window_size': WINDOW_SIZE
                }
            }, 'models/best_transformer_v2.pt')
    ...
    checkpoint = torch.load('models/best_transformer_v2.pt', weights_only=False)
    ...
    with open('models/transformer_v2_final_results.json', 'w') as f:
        json.dump({
            'final_metrics': {k: float(v) if not np.isnan(v) else None
                              for k, v in final_metrics.items()},
            'best_epoch': best_epoch,
            'best_wape': float(best_wape),
            'model_type': 'TransformerBaseline',
            'history': {k: [float(x) for x in v] for k, v in history.items()}
        }, f, indent=2)

    print("\n✅ Results saved to models/transformer_v2_final_results.json")
